...
p.addRequired('classPatch', @iscell);
p.addRequired('classTgt', @iscell);
...
for ii = 1 : length(classPatch)
    % Set the index for training and validation
    nSample = size(classPatch{ii}, 1);              % Total number of samples
    ...
    [kernels, ~, lssmse] = lscov(classPatch{ii}(indexTrain, :), classTgt{ii}(indexTrain, :));
    lssResidual = classPatch{ii} * kernels - classTgt{ii};
    ...
    relLssResidual = absLssResidual ./ classTgt{ii} * 100;
 ...
 %% Plot the results,showing the residual
    ...
    linearModelCh1 = fitlm(classPatch{ii}(indexTrain,:), classTgt{ii}(indexTrain,1), 'linear');
    wb1 = linearModelCh1.Coefficients.Estimate;
    w1 = wb1(2:end, 1);
    b1 = ones(size(classPatch{ii}, 1), 1) * wb1(1, 1);
    lmRMSECh1 = linearModelCh1.RMSE;

    linearModelCh2 = fitlm(classPatch{ii}(indexTrain,:), classTgt{ii}(indexTrain,2), 'linear');
    wb2 = linearModelCh2.Coefficients.Estimate;
    w2 = wb2(2:end, 1);
    b2 = ones(size(classPatch{ii}, 1), 1) * wb2(1, 1);
    lmRMSECh2 = linearModelCh2.RMSE;

    linearModelCh3 = fitlm(classPatch{ii}(indexTrain, :), classTgt{ii}(indexTrain,3), 'linear');
    wb3 = linearModelCh3.Coefficients.Estimate;
    w3 = wb3(2:end, 1);
    b3 = ones(size(classPatch{ii}, 1), 1) * wb3(1, 1);
    ...
    classRegressLM = classPatch{ii} * w + b;
    lmResidual = classTgt{ii} - classRegressLM;
