...
saveFolder = fullfile(L3rootpath, 'Cameras', 'basic_1strun');
...
for cameraFilenum = 1:length(cameraFiles)
    ...
    if length(cameraFile>9) & strcmp(cameraFile(1:9), 'L3camera_')
        ...
        camera = cameraSet(camera,'name',['Basic camera modified from ',cameraFile]);
        camera = cameraSet(camera,'vci name','default');

        % Remove from camera the L3 information and any metrics
        ...
        saveFile = fullfile(saveFolder, ['basiccamera_', namesuffix]);
