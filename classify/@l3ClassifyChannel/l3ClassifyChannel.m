classdef l3ClassifyChannel < l3ClassifyS
    ...
    methods (Access = public)
        ...
        function obj = l3ClassifyChannel(varargin)
        ...
        function labels = classify(obj, l3d, isNew, varargin)
            ...
            n_lvls = prod(cellfun(@(x) length(x), obj.cutPoints) + 1);
            
            if isNew || isempty(obj.p_data)
                obj.p_data = cell(n_lvls, nc);
                obj.p_out  = cell(n_lvls, nc);
            end
            ...
            for ii = 1:nImg
                ...
                pTypeC = pType(pdSz(1)+1:end-pdSz(1), ...
                               pdSz(2)+1:end-pdSz(2));
                curLabel = patchLabelsChannel(obj.cutPoints, stat, pTypeC(:), nc);
                
                % compute overall label values
                % labels{ii} = reshape(curLabel,size(pType)-obj.patchSize+1);
                labels{ii} = reshape(curLabel, [size(pType)-obj.patchSize+1, 2]);
                ...
                for jj = 1 : length(labelValue)
                    for kk = 1 : nc
                        ...
                        indx = patchFindIndex(curLabel, lv, pTypeC(:), kk);
                        ...
                        if size(d, 2) <= round(obj.p_max / nImg)
                            indx_d = 1 : size(d, 2);
                            obj.p_data{lv, kk} = [obj.p_data{lv, kk} d];
                        else
                            indx_d = randperm(size(d,2),round(obj.p_max/nImg));
                            obj.p_data{lv, kk} = [obj.p_data{lv, kk} d(:,indx_d)];
                        end
                        ...
                        if ~isempty(tgt)
                            ...
                            tgtD = tgtD(indx, :)';
                            obj.p_out{lv, kk} = [obj.p_out{lv, kk} tgtD(:, indx_d)];
                        end
    ...
    methods
        ...
        function val = get.nChannelOut(obj)
            ...
            val = nan;
            for ii = 1 : size(obj.p_out, 1)
                for jj = 1 : size(obj.p_out, 2)
                    if ~isempty(obj.p_out{ii, jj})
                        val = size(obj.p_out{ii, jj}, 1);
                        return
                    end
                end
            end
        ...
        function val = get.nLabels(obj)
            val = nan;
            if ~isempty(obj.p_data), val = numel(obj.p_data); end
        end
