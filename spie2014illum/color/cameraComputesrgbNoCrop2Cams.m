function [srgbResult, srgbIdeal, raw, camera, xyzIdeal, lrgbResult, srgbResult2, camera2, lrgbResult2] = ...
    cameraComputesrgbNoCrop2Cams(camera, sceneName, meanLuminance, sz, scenefov, ...
    scaleoutput,plotFlag,illum,camera2)
...
if nargin < 4 || isempty(sz)
    ...
    camera2 = cameraSet(camera2,'sensor size',sz + 20);
...
camera2.oi = camera.oi;
camera2.sensor = camera.sensor;
[camera2, lrgbResult2] = cameraCompute(camera2,scene,[],false);%,lrgbIdeal);   % OI is already calculated
srgbResult2 = lrgb2srgb(ieClip(lrgbResult2./max(xyzIdeal(:)),0,1));
